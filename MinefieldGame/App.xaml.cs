...
namespace MinefieldGame
{
    public partial class App : Application, IDisposable
    {
        ...
        private void AppStartup(object? sender, StartupEventArgs e)
        {
            ...
            {
                throw new Exception("ERROR: you need to have the res/mine.png file next to the executable!");
            }
            ...
            newGameButton = CreateButton("New game", (1280.0 - 280.0) / 2.0, 150.0, NewButtonAction);
            loadGameButton = CreateButton("Load game", (1280.0 - 280.0) / 2.0, 300.0, LoadGameAction);
            exitGameButton = CreateButton("Exit game", (1280.0 - 280.0) / 2.0, 450.0, ExitButtonAction);
            saveGameButton = CreateButton("Save game", (1280.0 - 280.0) / 2.0, 150.0, SaveGameAction);
            menuGameButton = CreateButton("Main menu", (1280.0 - 280.0) / 2.0, 450.0, MenuButtonAction);
            ViewStateUpdated(null, viewModel.ViewState);

            Rectangle r = CreateDrawable(submarineImage, new Point2D(100, 100), new Point2D(200, 100));
        ...
        {
            var tbr = new List<UIElement>();
            foreach (var item in canvas.Children)
            {
                if (item is UIElement ue && ue is not Button)
                {
                    tbr.Add(ue);
                }
            }

            foreach (var item in tbr)
            {
